...
const ProjectSection = () => {
  ...
  const updateDisplayedProjects = (category) => {
    if (category === 'ВСЕ') {
      const filteredProjects = allProjects.filter((project) => project.activity === 'production');
      setDisplayedProjects(filteredProjects);
    } else if (category === 'УЛИЦА') {
      const filteredProjects = allProjects.filter((project) => project.activity === 'production' && project.place === 'street');
      setDisplayedProjects(filteredProjects);
    } else if (category === 'ПОМЕЩЕНИЕ') {
      const filteredProjects = allProjects.filter((project) => project.activity === 'production' && project.place === 'room');
      setDisplayedProjects(filteredProjects);
    } else if (category === 'ВИДЕО') {
      const filteredProjects = allProjects.filter((project) => project.activity === 'production' && project.video !== 'none');
      setDisplayedProjects(filteredProjects);
    }
  };
  ...
  const handleButtonClick = (buttonText) => {
    ...
    // Вызываем функцию для фильтрации и обновления отображаемых проектов
    updateDisplayedProjects(buttonText);
  };

  useEffect(() => {
    // Отправляем запрос к API Sanity, чтобы получить данные о проектах
    client
      .fetch(`*[_type == 'project'] {
        name,
        image,
      ...
      .then((data) => {
        setAllProjects(data);
        const filteredData = data.filter((project) => project.activity === 'production');
